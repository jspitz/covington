...
% Titling
%
\def\pversion{Version 2.8dev}
\def\pdate{July 26, 2022}
...
\item{\joption{ownfncounter}:} Use a separate counter for numbered examples in footnotes.
	...
	By default, \cvt\ numbers examples in footnotes subsequently (with the numbering used in the main text). With this option,
	examples in footnotes get a different numbering, using by default roman numbering (see sec.~\ref{sec:custno} on how to change this).

\item{\joption{resetownfncounter}:} If a separate counter for numbered examples in footnotes is used, reset at each footnote.

	The roman nubering used with \joption{ownfncounter} is not reset between footnotes by default. Use this option if you prefer that.
\end{description}
%
...
\begin{itemize}
	\item Add \joption{ownfncounter} option to allow for separate counting of examples in footnotes. See sec.~\ref{sec:intro}.
	\item Add \joption{resetownfncounter} option to reset separate counters in each footnote. See sec.~\ref{sec:intro}.
\end{itemize}
