...
However, there are some notable exceptions due to the fact that we are dealing with multiple examples in a row
here:
\begin{itemize}
	\item \joption{judge} and its shorthand aliases (\joption{*}, \joption{?}, \joption{*?}, \joption{??}, and
		  \joption{\#}) do not have any effect since judgments only apply to specific items in the set.
		  Use the aforementioned angular \jcsmacro{item} option instead. 
	\item For the same reason, \joption{preamble} and \joption{postamble[*]} do not have any effect; 
		  if you want to add preceding or trailing text, you will want to add this to specific items in the set.
\end{itemize}
%
For the latter task, we provide specific macros, namely \jcsmacro{expreamble} and \jcsmacro{expostamble}, which can be placed
...
\end{description}
%
For convenience, both these options actually also work (as aliases) in optional arguments passed to
...
\begin{description}
	...
	\item[\joption{judge=\{\jparam{arbitrary text}\}}] Insert grammaticality judgments (such as * or ?)
		to a the gloss. This will be inserted before the first gloss line with a small space and uses its own
		markup. For convenience, the following shorthand options are also provided:
		\joption{*}, \joption{?}, \joption{*?}, \joption{??}, and \joption{\#} (as alias to
		\joption{judge=*} etc.).
	\item[\joption{fsjudge=\{\jparam{font settings}\}}] Default: \verb|{\normalfont\upshape}|.
...
\begin{description}\setlength\itemsep{0pt}
	...
	\item[\jcsmacro{xgll}] is similar to \jcsmacro{gll} except that it does not make
	       ...
	       This macro takes an optional argument where a judgment marker can be passed. 
