...
\begin{itemize}
	\item \joption{judge} and its shorthand aliases (\joption{*}, \joption{?}, \joption{*?}, \joption{??}, and
		  \joption{\#}) set the default judgment marker for all items in the environment. It also sets the appropriate
		  \joption{judgewith} for the selected marker.
...
\begin{description}
	...
	\item[\joption{tlr*=\jparam{true|false}}] Default: \emph{false}. A variant of \joption{tlr} which is
	     suitable for setting real free translations (enquoted) right to the gloss. the place of
	     gloss line comments and also the widths described below.
	...
	\item[\joption{fspostamble=\{\jparam{font settings}\}}] Default: \verb|{\normalfont\upshape}|.
     ...
     \item[\joption{glosswidth=\{\jparam{length}\}}] Default: \verb|{0.6\textwidth}|.
		 Sets the maximum width the gloss takes if optional comments are used.
     \item[\joption{glosssep=\{\jparam{length}\}}] Default: \verb|{1em}|.
		 Sets space between gloss and optional comments.
     \item[\joption{glosscommentwith=\{\jparam{length}\}}] 
		 Sets the maximum width of gloss comments. By default this is automatically calculated from the above
		 two lengths.
