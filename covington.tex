...
\def\pdate{April 1, 2017}

\title{\textbf{Typing Linguistics with \texttt{covington}}}
...
of \texttt{covington} (\pdate), which is a \LaTeX\ package providing macros
...
Eventually, it has been renamed to \texttt{covington} and adapted to \LaTeXe. Its \LaTeX\ 2.09 traces
are however still visible, and the style should actually still work with \LaTeX\ 2.09 (if not, drop us a note).}

To use \texttt{covington} with \LaTeXe, simply add the command
...
access to \LaTeX\ manuals. Note that \texttt{covington} does not 
provide any special fonts or character sets.  However, it can be used in 
combination with other style sheets that do.

If you are using \texttt{covington} and \texttt{uga.sty} (UGa thesis style) 
...
accents on the same letter.  To fill this gap, \texttt{covington} provides
...
\glend
\end{lstlisting}
Note that \texttt{covington} locally activates the end of line in glosses in order to identify the different lines of the gloss. This does not work inside macros (e.\,g., if the gloss is in a footnote). To workaround this, a special version of the \jcsmacro{gll} macro is provided that does without the character activation: \jcsmacro{xgll}. This can also be used in macro arguments; however, the end of each gloss line need to be explicitly specified by the \jcsmacro{xgle} macro in this case. If you want to put the above gloss in a footnote, thus, you would type:
...
All together, \texttt{covington} provides eight macros for dealing with
glosses:
\begin{itemize}
...
Several macros in \texttt{covington} facilitate display of discourse 
...
be changed by re-setting the length \jcsmacro{reflistindent}.
Doing \lstinline|\setlength\reflistindent{1.5em}|, for instance,
will shorten the indentation by half. Likewise, the length \jcsmacro{reflistsep}
(6\,pt by default) can be adjusted to alter the vertical separation of reference
entries.
...
Suggestions for improving \texttt{covington} are welcome, and bug
...
\begin{itemize}
	...
	\item New lengths \jcsmacro{reflistindent} and \jcsmacro{reflistsep}
	      to adjust the indentation or vertical space, respectively, of reflist items.
