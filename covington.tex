...
\Cvt\ provides two gloss macros, both also with a starred variant:
\begin{itemize}
	\item \jcsmacro{gloss\{\jparam{gloss line 1}\}\{\jparam{gloss line 2}\}\{\jparam{free translation}\}}\\
	       typesets two-line glosses with a translation line
	\item \jcsmacro{glosss\{\jparam{gloss l.\,1}\}\{\jparam{gloss l.\,2}\}\{\jparam{gloss l.\,3}\}\{\jparam{free translation}\}}\\
	       ...
	       See sec.~\ref{sec:glossex} for examples.}
       	   glosses with a translation line
\end{itemize}
%
The starred variants, \jcsmacro{gloss*} and \jcsmacro{glosss*}, do not add
a linebreak before the translation line. This can be useful when no translation,
but an aligned number, is to be inserted (please refer to sec.~\ref{sec:glossex}
for an example). The \jparam{free translation} argument can also be empty.
In this case, no translation line is added.

Note that the macros automatically markup the lines. By default, the first gloss
...
this package is loaded). This can be customized, though (see sec.~\ref{sec:glossfc}).

The example given above would thus be typed as:
\begin{lstlisting}[moretexcs={gloss}]
\gloss{Dit is een Nederlands voorbeeld}
      {This is a Dutch example}
      {This is an example in Dutch.}
\end{lstlisting}
...
\end{lstlisting}
%
Please read sec.~\ref{sec:glossex} for more examples.

\subsection{Glossing low-level commands}\label{sec:glosscmds}
...
in all cases (as documented in what follows). Thus, we strongly suggest to use the macros,
...
\end{lstlisting}
The markup of the translation line (if the \jcsmacro{gloss} or \jcsmacro{glosss}
...
instead of just two:
\glosss{Hoc est aliud exemplum}
       {\textsc{n.sg.nom} \textsc{3sg} \textsc{n.sg.nom} \textsc{n.sg.nom}}
       {This is another example}
       {This is another example.}
This is typed as:
\begin{lstlisting}[moretexcs={glosss}]
\glosss{Hoc est aliud exemplum}
       {\textsc{n.sg.nom} \textsc{3sg} \textsc{n.sg.nom} \textsc{n.sg.nom}}
       {This is another example}
       {This is another example.}
\end{lstlisting}
or in a more convenient way, using the font customization possibilities described
in sec.~\ref{sec:glossfc}:
\begin{lstlisting}[moretexcs={glosss,glosslinetwo}]
\begingroup
\renewcommand*\glosslinetwo{\normalfont\scshape}
\glosss{Hoc      est aliud    exemplum}
...
\endgroup
\end{lstlisting}
(with \lstinline|\begingroup...\endgroup| to keep the changes local).

Next, an example with a gloss but no translation, with an example number 
at the right:
\gloss*{Hoc habet numerum}
       {This has number}
       {\hfill (\exampleno)}
%
That one was typed using the starred version of the macro:
\begin{lstlisting}[moretexcs={gloss,exampleno}]
\gloss*{Hoc habet numerum}
       {This has number}
       {\hfill (\exampleno)}
\end{lstlisting}
...
\begin{itemize}
	\item Add new gloss macros (\jcsmacro{gloss}, \jcsmacro{gloss*}, \jcsmacro{glosss}, and \jcsmacro{glosss*})
