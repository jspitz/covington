...
environment as opptional arguments or globally via the \jcsmacro{setexampleoptions} macro.
...
\end{itemize}
and pass either of the above options to it. This will apply to all subsequent examples (the \jenv{example} environment
described here as well as \jenv{examples}, \jenv{subexamples} and glosses with the \joption{ex} option) until further
global change and unless the setting is altered locally via macro option.
...
where they should appear, and which take the same markup than the texts passed via \joption{preamble} or \joption{postamble}
option in the \jenv{example} environment.

Hence, also here, the advantage over just adding text as is in the example is that an own, globally settable markup, is used.
This is particularly relevant if you globally set examples to be italicized via the \joption{fs} option.
...
For a somewhat practical example, observe:
...
\end{subexamples}
%
The \jenv{subexamples}/\jenv{covsubexamples} environment provides the same options than the example and examples
environment (see sec.~\ref{sec:ex}) with the following deviations and additions:
\begin{description}
...
\begin{description}
	...
	\item[\joption{fssubpostamble=\{\jparam{font settings}\}}] Default: \verb|{\normalfont\upshape}|.
		Adjusts the font settings of subexample's  postamble text and the \jcsmacro{subexpostamble}
		macro described below.
		Valid values are \LaTeX\ font switches such as \jfcsmacro{itshape}, \jfcsmacro{bfseries} etc.
\end{description}
%
For convenience, both these options actually also work (as aliases) as optional arguments.
...
For this section we globally set the markup of example sentences to italics, as common in linguistics,
via
...
can be adjusted by means of the \joption{addnumbersep} example option.
...
\item My numbers are being moved.
\item Mine as well.
\end{subexamples}
\end{lstlisting}
...
the \textsf{enumerate} package \cite{enumerate}. In these templates, the first occurrence of the characters \emph{1}, \emph{i},
...
For instance, \jparam{(1)} is an alphabetic number in parentheses, \jparam{(a)} a lower alphabetic value, etc.
...
\joption{fspreamble}, \joption{fspostamble}, etc.
