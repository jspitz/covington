...
\begin{description}
	...
	\item[\joption{leftmargin=\{\jparam{length}\}}] Default: \verb|{0pt}|.
	 ...
	 \item[\joption{numberformat=\{\jparam{template}\}}] Default: \verb|{(1)}|.
	    ...
	    \jparam{a} and \jparam{A}. Any text before and after the number are inserted as is, e.\,g.,
	    \verb|numberformat={[a.]}| results in lower alphabetic numbering followed by a dot, embraced in brackets.
...
\end{examples}
%
The \jenv{examples} environments accepts the same options than the \jenv{example} environment. Only \joption{preamble}
and \joption{postamble[*]} do not have any effect.
...
\begin{description}
	...
	\item[\joption{fspostamble=\{\jparam{font settings}\}}] Default: \verb|{\normalfont\upshape}|.
		...
		Valid values are \LaTeX\ font switches such as \jfcsmacro{itshape}, \jfcsmacro{bfseries} etc.
	\item[\joption{leftmargin=\{\jparam{length}\}}] Default: \verb|{0pt}|.
		Lets you change the indentation of subexamples (including number). Positive values will increase,
		negative values will decrease the indentation of the example.
	\item[\joption{addnumbersep=\{\jparam{length}\}}] Default: \verb|{0pt}|.
		Increases or (with negative values) decreases the spacing between example number and subnumber.
	\item[\joption{addsubnumbersep=\{\jparam{length}\}}] Default: \verb|{0pt}|.
		Increases or (with negative values) decreases the spacing between example subnumber and example.
	\item[\joption{numberformat=\{\jparam{template}\}}] Default: \verb|{(1)}|.
		The format of the example number. The option takes templates as known from the \textsf{enumerate}
		package \cite{enumerate}.
		Arabic numbering is represented by \jparam{1}, Roman by \jparam{i} and \jparam{I}, alphabetic by
	...
	\item[\joption{subnumberformat=\{\jparam{template}\}}] Default: \verb|{(a)}|.
		The format of the sub-example number. For the template format, see \joption{numberformat}.
\end{description}
%
For instance,
\begin{lstlisting}
...
All example environments can be customized in many aspects via the example options. If passed as an
optional argument to the respective environment, the change only applies to this very environment.
If passed via \jcsmacro{setexampleoptions}, the change will apply to any subsequent example environment
unless a local option overrides a setting.
...
you can increase it by means of the \joption{addnumbersep} example option.

If you need to change the indentation (left margin) of examples and subexamples, you can do so by means of
the \joption{leftmargin} example option. Doing \joption{leftmargin=1em},
for instance, will increase the indentation by 1\,em, negative values will decrease it accordingly.
So, for instance,
\begin{lstlisting}[moretexcs={leftmargin,addnumbersep}]
\begin{example}[leftmargin={-2.2em},addnumbersep={-0.4em}]
...
\end{example}
%
\noindent In case of \jenv{subexamples}, the distance between example subnumber and text can be changed
...
You can easily change the display of the example number or subnumber globally or locally for only one
occurrence via the example options. These take a `mini template' as known from the \textsf{enumerate}
package \cite{enumerate}. In these templates, the first occurrence of the characters \emph{1}, \emph{i},
\emph{I}, \emph{a}, and \emph{A} represents the numeric system where the respective character marks the first positive
value. All the rest of the template (including following occurrences of the mentioned characters) is interpreted literally.
For instance, \emph{(1)} is an alphabetic number in parentheses, (a) a lower alphabetic value, etc.

So if you want the example number to have a dot after the number, simply use the option \joption{numberformat=\{(1.)\}}
To remove the parentheses from the subexample letter, for instance, use \joption{subnumberformat=\{a\}}:
...
Example markup can be customized via the \joption{fs*} example options. This way, all elements
of examples (number, actual example text, pre- and postamble) can be marked up independently
from each other. Just to showcase the most common option, the example in italics. This can
be done by globally or locally setting \verb|fs=\itshape|, as in:
\begin{lstlisting}[moretexcs={covsubexnumber}]
\begin{example}[fs=\itshape,preamble={This is the preamble},
                postamble={(Postamble)}]
This is the example.
\end{example}
\end{lstlisting}
...
\end{example}
%
Analogously, you can adjust the example number font (\joption{fsno}), the preamble font
(\joption{fspreamble}), and the postamble font (\joption{fspostamble}).
...
\end{lstlisting}
	     ...
	     for the \joption{enquotetl} option of the gloss macro. The macro \jcsmacro{cov@fstl} holds the font
	     settings as set via the \joption{fstl} gloss option. These are both relevant also for low-level
...
\begin{itemize}
	...
	\item New example options: \joption{fs}, \joption{fsno}, \joption{fspreamble}, \joption{fspostamble},
		  \joption{leftmargin}, \joption{addnumbersep}, \joption{addsubnumbersep}, \joption{numberformat},
		  \joption{subnumberformat}, \joption{fnnumberformat} (see sec.~\ref{sec:ex}).
